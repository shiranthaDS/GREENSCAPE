...
const AdminPage = () => {
  ...
  const handleUpdateStatus = async (id, status) => {
    try {
      await axios.put(`http://localhost:5000/api/jobApplications/updateStatus/${id}`, { status });
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/api/jobApplications/delete/${id}`);
          ...
          {currentRows.map((application) => (
            <tr key={application._id}>
              ...
              <td>
                {application.resume && (
                  <a
                    href={`http://localhost:5000/api/uploads/${application.resume}`}
                    download
                  >
                    Download
                  </a>
